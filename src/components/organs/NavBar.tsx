...
import { connectWallet } from '../../metamask/metamask'; // Import the functions


const NavBar: React.FC = () => {

    const [walletAddress, setWalletAddress] = useState<string | null>(null); // Initialize with null
        ...
        <header className="w-full h-auto bg-transparent overflow-x-hidden fixed z-50 top-0 left-0">
            <Slide direction="down">
                <nav className={`w-full md:h-24 h-20 ${navBarColor ? "bg-white" : "bg-transparent"} lg:px-24 md:px-12 px-8 flex justify-between items-center`}>
                    <Image as="a" href="/" className="md:h-10 h-8" image={Logo} alt="Logo"  style={{width:'150px'}}/>
                        ...
                        <ul className="flex items-center justify-center gap-6">
                            {NavButtons.map((navbutton, index) => (
                                <List className="w-full" key={index}>
                                    <Button
                                        onClick={toggleWallet} // Toggle function
                                        type="button"
                                        className={`${
